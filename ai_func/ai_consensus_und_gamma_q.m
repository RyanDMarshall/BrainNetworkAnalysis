function [ciu, avgQ] = ai_consensus_und_gamma_q(d,tau,reps,gamma)
%Alex: Script was modified to also take gamma
%
%CONSENSUS      consensus clustering
%
%   CIU = CONSENSUS(D,TAU,REPS) seeks a consensus partition of the 
%   agreement matrix D. The algorithm used here is almost identicabinAgrYA1l to the
...
while flg == 1
    ...
    if nnz(dt) == 0
        ...
        Q = zeros(1,reps); % ai
        parfor iter = 1:reps
            [ci(:,iter), Q(iter)] = community_louvain(dt,gamma); % ai
        end
